...
class RealmWriteServiceTest {
    ...
    @InjectMocks
    private RealmWriteService service;
    ...
    void initialize_shouldFetchAndSaveMissingRealms() {
        ...
        given(realmReadRepository.findSlugByRegion(region)).willReturn(existingSlugs);
        ...
        given(realmWriteRepository.saveAll(Set.of(expected1, expected2))).willReturn(List.of(expected1, expected2));
        ...
        verify(realmReadRepository).findSlugByRegion(region);
        verify(realmFetcher).fetchAllRealmsAsync(region, allSlugs);
        verify(realmWriteRepository).saveAll(Set.of(expected1, expected2));
    }
