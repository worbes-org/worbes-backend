package com.worbes.adapter.blizzard.data.realm;

import com.worbes.application.realm.model.Realm;
import com.worbes.application.realm.model.RegionType;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.mapstruct.factory.Mappers;

import java.util.Map;

import static org.assertj.core.api.Assertions.assertThat;

class RealmResponseMapperTest {

    private final RealmResponseMapper mapper = Mappers.getMapper(RealmResponseMapper.class);

    @Test
    @DisplayName("RealmResponse를 RealmFetchResult로 올바르게 매핑한다")
    void shouldMapRealmResponseToDto() {
        // given
        RealmResponse response = new RealmResponse();
        response.setId(100L);
        response.setName(Map.of("ko-KR", "아즈샤라"));
        response.setConnectedRealmHref("https://api.test/connected-realm/1");
        response.setSlug("test-realm");

        RegionType region = RegionType.KR;

        // when
        Realm dto = mapper.toDomain(response, region);

        // then
        assertThat(dto).isNotNull();
        assertThat(dto.getId()).isEqualTo(response.getId());
        assertThat(dto.getName()).isEqualTo(response.getName());
        assertThat(dto.getConnectedRealmId()).isEqualTo(response.getConnectedRealmHref());
        assertThat(dto.getSlug()).isEqualTo(response.getSlug());
        assertThat(dto.getRegion()).isEqualTo(region);
    }
}
