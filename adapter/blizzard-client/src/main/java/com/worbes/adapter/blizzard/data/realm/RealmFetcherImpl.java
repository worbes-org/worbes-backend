...
public class RealmFetcherImpl implements RealmFetcher {
    ...
    @Override
    public CompletableFuture<List<RealmFetchResult>> fetchAllRealmsAsync(RegionType region, Set<String> slugs) {
        ...
        return CompletableFuture.allOf(futures.toArray(new CompletableFuture[0]))
                .thenApply(v -> futures.stream()
                        ...
                        .map(response -> realmResponseMapper.toDto(response, region))
                        .toList()
                );
    }
