...
class BlizzardAccessTokenHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        given(properties.getTokenKey()).willReturn(tokenKey);
    ...
    void shouldReturnCachedTokenWithoutRefresh() {
        ...
        then(restApiClient).should(never()).post(any(RequestParams.class), eq(BlizzardTokenResponse.class));
    }
    ...
    void shouldRefreshTokenAndSaveToCache() {
        ...
        then(restApiClient).should().post(any(RequestParams.class), eq(BlizzardTokenResponse.class));
    }
